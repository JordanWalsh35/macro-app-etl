...
fci = load_table("Fed FCI")
fci = fci[fci.index > "2000-01-01"]
# Add in ISM YoY%
ism_df["ISM YoY%"] = ism_df["ISM"].pct_change(periods=12) * 100
ism = ism_df[["ISM YoY%"]]
ism = ism[ism.index>"2007-01-01"]
...
with col2:
    ...
    fig1 = plot_datasets(primary_df=ism, secondary_df=usd, primary_series="ISM YoY%", secondary_series="USD YoY%", primary_range=[-32, 45], secondary_range=[-20, 12])
    ...
    fig2 = plot_datasets(primary_df=ism, secondary_df=wti, primary_series="ISM YoY%", secondary_series="WTI Crude YoY%", primary_range=[-32, 45], secondary_range=[-70, 130])
    ...
    fig3 = plot_datasets(primary_df=ism, secondary_df=rates, primary_series="ISM YoY%", secondary_series="US 10YR YoY%", primary_range=[-32, 45], secondary_range=[-80, 150])
    ...
    fig4 = plot_datasets(primary_df=ism, secondary_df=spreads, primary_series="ISM YoY%", secondary_series="HY Credit Spreads YoY%", primary_range=[-32, 38], secondary_range=[-150, 130])
    ...
    fig5 = plot_with_constant(df=fci, series_name="Chicago Fed NFCI", constant_y=0)
    ...
    fig6 = plot_with_constant(df=fci, series_name="FCI Leverage", constant_y=0)
    ...
    fig7 = plot_with_constant(df=fci, series_name="FCI Credit", constant_y=0)
